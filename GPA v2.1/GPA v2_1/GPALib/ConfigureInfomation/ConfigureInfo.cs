...
namespace GPA.GPALib.ConfigureInfomation
{
     ...
     class ConfigureInfo
    {
        ...
        private XmlNode _tableNode; //记录需要导入实验室Sql Server数据库中的表
        ...
        public string ProgramBeginTime {
            get { return Convert.ToDateTime(_programRunTime.Attributes["begin_time"].Value).ToString() ; }
        ...
        public void Load(string xPath)
        {
            ...
            try
            {
                ...
                _tableNode = _node.SelectSingleNode("importTable");
         ...
         /// <summary>
         /// 获取需要导入实验室Sql Server数据库的列表
         /// </summary>
         /// <returns></returns>
         public List<string> GetImportTableList()
         {
             List<string> tableList = new List<string>();
            foreach(XmlNode node in _tableNode.ChildNodes)
            {
                try{
                    tableList.Add(node.Attributes["name"].Value);
                }
                catch(Exception)
                {}
            }
