...
{
    class DbServerForeignEmuleData:DbServerDataBase
    {
        public DbServerForeignEmuleData(ref DbBase dbBase, DateTime beginTime, DateTime endTime, bool isPartition) :
        ...
        protected override string GetCommand()
        {
            ...
            else
            {
                    strSql = string.Format(@"select distinct server_ip,{0}
                     from {1}  where type='em'  and (peer_source='tracker'or peer_source='server') and asciistr(resource_uri) not like '%\%'
