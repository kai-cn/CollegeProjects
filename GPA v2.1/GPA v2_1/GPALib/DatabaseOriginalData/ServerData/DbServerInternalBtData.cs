...
{
    class DbServerInternalBtData:DbServerDataBase
    {
        public DbServerInternalBtData(ref DbBase dbBase, DateTime beginTime, DateTime endTime, bool isPartition) :
        ...
        protected override string GetCommand()
        {
            ...
            else
            {
                ...
                strSql = string.Format(@"select distinct server_ip,{0}
                     from {1}   where  (peer_source='tracker'or peer_source='server' or peer_source is null) and  regexp_like(SUBSTR(resource_uri,LENGTH(resource_uri)-48+1,48),'[a-z|0-9]{{40}}.torrent')
