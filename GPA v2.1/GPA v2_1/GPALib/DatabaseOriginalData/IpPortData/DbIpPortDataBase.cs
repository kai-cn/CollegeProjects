...
{
    /// <summary>
    /// 为ip分布,country分布,port分布等存储原始数据
    /// </summary>
    abstract class DbIpPortDataBase:DbOriginalDataBase
    {
        protected List<ADT.OriginalDataUnit> _originalDataLists;

        
        public DbIpPortDataBase(ref DbBase dbBase, DateTime beginTime, DateTime endTime, bool isPartition) :
        ...
        public override object GetOriginalData()
        {
            ...
            if (ds != null)
            {
                ...
                try
                {
                    ...
                    {
                        i++;
                        _originalDataLists.Add(new ADT.OriginalDataUnit(dr[0].ToString(), Convert.ToDateTime(dr[1]),dr[2].ToString()));
                        if (i % 5000 == 0)
                            Thread.Sleep(200);
